...
def cmdJudgFunc(cmdInput, page):
    ...
    # 부트캠프 판단
        ...
        if(succInputJudgFunc(cmdInput) == True):
            strOutputFunc(strBootCampLocation)
...
def fightFunc(listMonsterAll, playerSiteInfo, situation):
    ...
    stmPrimePlayer = playerSiteInfo["스텟 모음"]["스태미나"]
    hpPrimeMonster1 = listMonster[0]["체력"]
    if len(listMonster) > 1:
    ...
    while True:
        ...
        # 전투시
            ...
            # 플레이어 턴
            strSeqOutputFunc("[SYSTEM] 플레이어의 턴 입니다.", timeDelay)
            healthBarOutputFunc(playerName, listMonster, playerSiteInfo, hpPrimePlayer, stmPrimePlayer, hpPrimeMonster1, hpPrimeMonster2)
            ...
            # 몬스터 턴
            strSeqOutputFunc("[SYSTEM] 적의 턴 입니다.", timeDelay)
...
# 체력바 출력 함수
def healthBarOutputFunc(playerName, listMonster, playerSiteInfo, hpPrimePlayer, stmPrimePlayer, hpPrimeMonster1, hpPrimeMonster2):
    # 이름 문자열 길이 변수
    wordLenPlayer = 0
    wordLenMonster1 = 0
